...
module.exports =
  fixEmailFormattedUsernames: wrap (req, res, next) ->
    users = yield User.find({nameLower: {$regex: filter}}).select({name:1, email:1, anonymous:1, slug:1}).limit(10)
    ...
    for user in users
      ...
      console.log 'checking user', user.id, JSON.stringify(user.toObject())
      ...
      if not oldEmail
        ...
        throw new errors.InternalServerError('Unhandled user')
